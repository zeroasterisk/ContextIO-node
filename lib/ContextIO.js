	...
	function _doCall(method, resource, params, cb, saveBodyToFileName) {
		...
		req.on('response', (saveBodyToFileName) ? function (r) {
			...
			r.on('end', function() {
				...
				if (body != '') {
					try {
						rBody = (r.headers['content-type'] == 'application/json') ? JSON.parse(body.replace(/^\s*/,'').replace(/\s*$/,'')) : body;
					} catch (e) {
						rBody = null;
					}
				}
        ...
        emailAddresses: function () {
          return this.email_addresses.apply(this, arguments);
					...
					return {
						...
						folders: function () {
							...
							return {
								get: function () {
									if (_folderName) {
										MethodNotForInstanceError('accounts/sources/folders', 'get', _parseArgs(arguments).cb);
									} else {
										_doCall.call(_obj, 'GET', _resourcePath, null, _parseArgs(arguments).cb);
									}
								},
				...
				threads: function () {
					...
					return {
						get: function () {
							_doCall.call(_obj, 'GET', _resourcePath + _threadId, null, _parseArgs(arguments).cb);
						}
					};
				},
